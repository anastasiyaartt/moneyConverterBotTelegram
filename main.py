...
bot = telebot.TeleBot('8119965192:AAFFPtxP_25iLyNbCRoFcKUoU8CsIX1m8Eo')
...
def start(message):
    bot.send_message(message.chat.id, 'Здравствуй, введи сумму')
...
def summa(message):
    ...
    if amount > 0:
       ...
       bot.send_message(message.chat.id, 'Выберите пару валют', reply_markup=markup)
        ...
        bot.send_message(message.chat.id, 'Чмсло должно быть больше 0. Впишите сумму')
        bot.register_next_step_handler(message, summa)
...
def my_currency(message):
    try:
      ...
      bot.register_next_step_handler(message, summa)
        ...
        bot.send_message(message.chat.id, f'Произошла ошибка. Впишите значение заново')
